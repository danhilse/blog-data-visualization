...
const TreemapNode = React.memo(({
      ...
      {width > 40 && height > 30 && (
        <animated.g
          style={{
            opacity: springProps.opacity,
            transform: to(
              [springProps.x, springProps.y, springProps.scale, springProps.width, springProps.height],
              (x, y, scale, width, height) => 
                `translate(${x + width / 2}px, ${y + height / 2}px) scale(${scale})`
            )
          }}
        >
          <Text
  ...
  if ((prevNode.opacityValue ?? prevNode.data.opacityValue) !== 
...
return false;
  ...
  if (prevProps.highlightedCluster !== nextProps.highlightedCluster) {
    const wasHighlighted = prevProps.highlightedCluster === prevNode.data.useCase;
    const isHighlighted = nextProps.highlightedCluster === nextNode.data.useCase;
...
const UseCaseTreemap = () => {
  ...
  const metricBounds = useMemo(() => ({
    min: allAnalytics.length ? Math.min(...allAnalytics.map(a => a[opacityMetric]).filter(v => v != null)) : 0,
    max: allAnalytics.length ? Math.max(...allAnalytics.map(a => a[opacityMetric]).filter(v => v != null)) : 1
  }), [allAnalytics, opacityMetric]);

  const calculateOpacity = useCallback((value, metric) => {
    ...
    return 0.2 + (0.8 * (value - metricBounds.min) / (metricBounds.max - metricBounds.min));
  }, [metricBounds]);
  ...
  const maxHeight = typeof window !== 'undefined' ? window.innerHeight * 0.78 : 800;
    ...
    <div className="w-full h-full bg-white rounded-xl shadow-lg p-6">
      ...
      >
        <svg width="100%" height={dynamicHeight}>
          <rect width="100%" height={dynamicHeight} fill={baseColors.background} rx={8} />
            <Treemap
            root={root}
