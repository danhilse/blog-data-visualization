...
import { useSpring, useSprings, animated, config, useSpringRef, useChain, to } from 'react-spring';
...
const ZoomableSunburst = () => {
  ...
  useChain(
    [rotationSpringRef, ring1SpringRef, ring2SpringRef, ring3SpringRef, ring4SpringRef],
    [0, 0.2, 0.4, 0.65, 0.82]
  );
  ...
  function renderNode(node, depth = 0) {
    ...
    const springForDepth = [ring1Spring, ring2Spring, ring3Spring, ring4Spring][depth];
    ...
    return (
      <AnimatedGroup key={`${node.name}-${depth}`}>
        <AnimatedPath
          d={to([springForDepth.progress, springForDepth.angle], (progress, angle) => {
            const interpolatedStartAngle = node.startAngle;
            const interpolatedEndAngle = depth === 0 ? node.endAngle : node.startAngle + (node.endAngle - node.startAngle) * angle;

            let currentStart = 0;
            for (let i = 0; i < depth; i++) {
              const prevRingCurrentProgress = [ring1Spring, ring2Spring, ring3Spring, ring4Spring][i].progress.get();
              ...
              transform: to(
                ...
                (progress, angle) => {
                  const [centroidX, centroidY] = arc().centroid({
                    ...
                    startAngle: node.startAngle,
                    endAngle: depth === 0 ? node.endAngle : node.startAngle + (node.endAngle - node.startAngle) * angle
                  });
                  const rotationAngle = ((node.startAngle + node.endAngle * angle) / 2) * (180 / Math.PI) - 90;
