...
const EnhancedSankeyChart = () => {
  ...
  const [data, setData] = useState<{ nodes: NodeDatum[]; links: LinkDatum[] } | null>(null);
  ...
  const getNodeBackground = (node: any) => {
    ...
    const category = node.nodeType === 'category' ? node.name : node.parentCategory;
    ...
    async function fetchData() {
      try {
        ...
        rawData.forEach((d: any) => {
          ...
          hierarchy[priority].aiCategories[aiCategory].useCaseCounts[useCaseName] =
            (hierarchy[priority].aiCategories[aiCategory].useCaseCounts[useCaseName] || 0) + 1;
        ...
        Object.entries(hierarchy).forEach(([priority, priorityData]) => {
          ...
          Object.entries(priorityData.useCases).forEach(([useCaseName, count]) => {
          ...
          Object.entries(priorityData.aiCategories).forEach(([aiCategory, aiData]) => {
            ...
            Object.entries(aiData.useCaseCounts).forEach(([useCaseName, count]) => {
              links.push({
  ...
  if (!data) return <div className="w-full h-full flex items-center justify-center">No data available</div>;
          ...
          >
            {({ graph, createPath }) => {
              ...
              const linkAnimations = useTrail(graph.links.length, {
                ...
                config: springConfig,
                delay: i => i * 5 + 100, //  Staggered, but very quick.
              });
              ...
              const nodeAnimations = useTrail(graph.nodes.length, {
                ...
                config: springConfig,
                delay: i => i * 10 + 300,  // Staggered, starts after links have begun.
              });
                  ...
                  {graph.links.map((link, i) => {
                    ...
                    return(
                      <g key={`link-${i}`}>
                        ...
                        onMouseEnter={(e) => {
                          e.target.style.filter = 'brightness(1.2) drop-shadow(0 0 8px rgba(255,255,255,0.3))';
                    ...
                    return (
                      <Group key={`node-${i}`}>
                        <animated.rect
                          ...
                          opacity={opacity.to(o => o * getNodeOpacity(node.nodeType))}
                          style={{ transform }}
                          onMouseMove={(e) => {
                            const coords = localPoint(e.target.ownerSVGElement, e);
                            showTooltip({
